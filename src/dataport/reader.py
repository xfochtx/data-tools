...
def excel(ruta: Path, **kwargs) -> pd.DataFrame:
  """Read an Excel file into a DataFrame.

  Args:
    ruta: Path to the .xlsx or .xls file.
    **kwargs: Passed through to pd.read_excel().
  """
  return pd.read_excel(ruta, **kwargs)


def csv(ruta: Path, **kwargs) -> pd.DataFrame:
